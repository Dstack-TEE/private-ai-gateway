...
/// The exact §10 refusal body bytes. Built once per refusal so the receipt's
/// `response.returned` hash covers the bytes actually served.
pub(super) fn refusal_error_body(error_type: &str, message: String) -> Vec<u8> {
    let body = json!({
        "error": {
            "message": message,
            "type": error_type,
