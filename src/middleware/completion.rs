...
pub async fn run(
    ...
    match result {
        ...
        Ok(MiddlewareForwardResult::Stream(forward)) => {
            // Normalized, not relayed: providers spell the SSE content type
            // differently (`text/event-stream` vs `; charset=utf-8`), and that
            // difference alone distinguishes backends. Anything that is not SSE
            // is kept as-is rather than mislabeled.
            let content_type = match forward.upstream_headers.get("content-type") {
                Some(value) if value.starts_with("text/event-stream") => {
                    "text/event-stream".to_string()
                }
                Some(value) => value.clone(),
                None => "text/event-stream".to_string(),
            };
            let upstream_status = forward.upstream_status;
            let attempt_index = forward.failed_attempts.len() as u32;
