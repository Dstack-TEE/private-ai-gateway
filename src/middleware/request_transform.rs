...
///
/// 2. `tools` present, no `response_format`: tool calls. Apply threshold only
///    — disable reasoning when max_tokens is small enough that reasoning might
///    eat the budget. Above the threshold, keep the caller's reasoning.
///
/// 3. `tools` + `response_format`: keep reasoning. The model needs reasoning
///    for tool call formatting when response_format constrains the output.
///
/// 4. Neither: normal chat. Keep the caller's reasoning.
fn resolve_effective_reasoning(
    ...
    if has_rf && !has_tools {
        ...
        policy
            ...
            .or(requested.cloned())
    } else if has_tools && !has_rf {
        // Tools only: threshold. Disable below threshold, keep caller above.
