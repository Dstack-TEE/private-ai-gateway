//! Immutable, content-addressed attested-session records (ACI §8).
//!
...
//! and evidence. Sessions are content-addressed over the JCS form of the
//! document (Appendix A):
//!
//! ```text
//! session_id = hex(sha256(JCS(session document)))
//! ```
//!
//! The id is not inside the document; the signed receipt commits to it, so a
//! relying party recomputes the id from the fetched document to prove the
//! record is exactly what the receipt cited. Sessions are never updated in
//! place — re-verification produces a new document, period, and id.
...
/// [`ClaimStatus::Refuted`]; an `Unknown` claim carries neither (§8.3).
...
/// The §8.3 typed claim vocabulary. Every field defaults to
...
/// The §8.2 evidence object: a `sha256:` digest over the decoded
...
/// The §8.2 session document — exactly the members the served bytes carry.
...
pub struct SessionDocument {
    ...
    /// (serving the record to relying parties) outlives this (§8).
    pub expires_at: u64,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// A session frozen with its JCS bytes. Sealed once; the store keeps and
/// serves those bytes, and `session_id` is the §8 content address over
/// them — any re-encoding of the same document recomputes the same id.
...
impl AttestedSession {
    ...
    pub fn seal(document: SessionDocument) -> Result<Self, serde_json::Error> {
        let bytes = digest::jcs_bytes(&serde_json::to_value(&document)?)
            .map_err(serde::ser::Error::custom)?;
        let session_id = digest::sha256_bare_hex(&bytes);
        Ok(Self {
            session_id,
            bytes,
            document,
    ...
    pub fn from_bytes(bytes: Vec<u8>) -> Result<Self, serde_json::Error> {
        ...
        let session_id =
            digest::sha256_bare_hex(&digest::jcs_bytes(&value).map_err(serde::ser::Error::custom)?);
        Ok(Self {
            session_id,
            bytes,
            document,
    ...
    /// Bare 64-hex over [`Self::bytes`].
    pub fn session_id(&self) -> &str {
        &self.session_id
    }

    /// The exact bytes `GET /v1/aci/sessions/{session_id}` must serve.
...
mod tests {
    ...
    fn session_id_is_the_hash_of_the_served_bytes() {
        ...
        assert!(session.session_id().bytes().all(|b| b.is_ascii_hexdigit()));
    ...
    fn any_document_change_changes_the_id() {
        ...
        // A new validity period is a new session (§8): timestamps are in the bytes.
