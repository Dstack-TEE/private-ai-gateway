...
//! satisfy the §3.3 custody rules: generated inside the attested workload,
...
/// The baseline signature algorithm every verifier implements; a keyset
/// MAY carry others, and the attested entry decides (§7.2, Appendix B).
pub const ALGO_ED25519: &str = "ed25519";
/// legacy `X-Signing-Algo` labels (not ACI algorithm names).
...
pub struct Quote {
    ...
    /// Used for the §9.1(4) compose check: `sha256(app_compose)` must equal
    /// the RTMR3 `compose-hash` event.
...
pub trait Quoter: Send + Sync {
    ...
    /// the vendor report-data slot per §3.2 (zero-padded to the slot width).
...
pub trait KeyProvider: Send + Sync {
    /// Attested Ed25519 receipt signing keys (§3.1).
    fn receipt_keys(&self) -> Vec<KeyedPublicKey>;

    /// Sign the exact receipt payload bytes (§7.2): a raw 64-byte RFC 8032
    ...
    /// legacy `X-Signing-Algo` compatibility keys (`ecdsa` / `ed25519`
    ...
    /// A compatibility surface, separate from ACI receipt signing.
    ...
    /// Optional provider-specific proof of key custody or key release (§3.3).
...
/// Verify an ACI receipt signature (§9.3): a raw RFC 8032 Ed25519 signature
