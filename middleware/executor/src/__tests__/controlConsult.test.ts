...
// controlConsult reads PRIVATE_AI_GATEWAY_CONTROL_URL / _TOKEN at module load,
// so a real loopback server is started and the env is set *before* the dynamic
// import below.
describe('controlConsult — remote dial', () => {
  let server: http.Server;
  let serverClosed = false;
  let received: { auth?: string; path?: string; body?: string }[] = [];
  let nextResponse = { status: 200, body: '{}' };
  let consultPre: typeof import('../controlConsult').consultPre;
  ...
  beforeAll(async () => {
    server = http.createServer((req, res) => {
      ...
      req.on('data', (c) => (b += c));
      req.on('end', () => {
        received.push({ auth: req.headers.authorization, path: req.url, body: b });
    ...
    ({ consultPre } = await import('../controlConsult'));
  });
  ...
  it('sends the Bearer token + content-blind body to /consult/pre and parses the result', async () => {
    ...
    expect(JSON.parse(received[0].body!)).toEqual({ apiKeyHash: 'abc', model: 'gpt-4o' });
  });

  it('fails closed (503) on a non-200 control response', async () => {
    nextResponse = { status: 500, body: 'boom' };
    const res = await consultPre('gpt-4o', 'abc');
    expect(res).toEqual({ allow: false, status: 503, message: 'control plane unavailable' });
  });
