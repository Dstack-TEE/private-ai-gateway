...
(`private-ai-proxy audit`) for ACI canonicalization, keyset binding, and receipt signature
...
- Fetches `/v1/aci/receipts/{receipt_id}` using the `x-receipt-id` header value as
  ...
  path parameter.
- Runs the same `private-ai-proxy audit` offline verification against the receipt +
...
Procedure (delegated to `private-ai-proxy audit`):
...
The output is the `private-ai-proxy audit --json` transcript: a `checks` array (id,
