...
| CLI verifier policy | Partial | User-configurable §1.3 policy for Private AI Proxy's ACI commands. Landed: required claims (`--require-claim`, §9.2(3)), attested-session pinning (`private-ai-proxy sessions`, `serve --session` / `--require-claim` with refresh-on-412), compose pins, and an RTMR3 `os-image-hash` production allowlist (`--require-production-os`). The OS allowlist relies on a separate dstack verifier to bind the hash to MRTD/RTMR0-2. Remaining: custody checking (§9.1(5)), in-client dstack boot verification, and exact source-build provenance policy. |
