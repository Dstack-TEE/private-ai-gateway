...
8. **Session validity is advertised far longer than a session can actually
    ...
    part of the channel fingerprint, so a refresh every
    `verifier_cache_seconds` (default 300) can supersede the old session while
    the list endpoint still advertises it as current until `expires_at`. A
    client that pinned it (§5.3) is then refused `session_not_accepted`.
    Fix direction: end a session's validity period when a re-verification
