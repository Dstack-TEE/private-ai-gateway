...
1. **The `pap` CLI has no default custody policy.** §9.1(5) requires the
   ...
   runs Gateway's custody check (the shared `aci-verifier` crate) only when
   given a policy — `--accept-dstack-kms-root-public-key` with
   `--accept-subject` or `--accept-image-digest` — and ships no default trust
   anchors, so without flags `pap verify` reports id-5 as an honest `skip`,
   never a pass. Managed profiles in the desktop app carry no custody policy
   yet.
