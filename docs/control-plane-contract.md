...
| `candidates[].reasoningFormat` | No | Upstream reasoning dialect: `reasoning_effort`, `reasoning`, `chat_template_thinking`, or `chat_template_enable_thinking`. When omitted, managed routes use `reasoning` and self-hosted engines use `reasoning_effort`. |
...
`errorSource` is the request-level summary after the candidate chain failed.
