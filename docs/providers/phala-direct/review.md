...
| `gpu_attested` | **Derived from CPU + serving software, not NRAS** | The sound source is the **CPU-TEE quote + measured serving software**: the serving software, attested inside the CPU-TEE quote, attests the GPU and establishes the encrypted CPU↔GPU CC channel, refusing to serve otherwise. So once the CPU-TEE gate passes, a GPU the attested software presents is verified by implication — `gpu_verified` is derived from `gpu_evidence_present`. **NRAS is not called** (it is only an existence oracle). The strength of this claim still rides on `serving_software_known_good` (see that row); GPU is never a gate. |
