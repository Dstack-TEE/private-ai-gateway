...
| Confidential AI | c8s front door on node-as-CVM (TDX) | Not acceptable | Production TLS key is host-visible (`webpki`); no signed production release; operator key and host-readable RKE2 state disk both reach cluster-admin inside the TEE. See [review](confidential-ai/review.md). |
