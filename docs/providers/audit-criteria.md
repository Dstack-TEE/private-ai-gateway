...
| Confidential AI | c8s front door and mesh on TDX node-as-CVMs | Not acceptable | Operator key and host-readable RKE2 state disk both reach cluster-admin inside the TEE (RTMR3 not zero); plaintext-path workloads admit `env: any` and `mounts: any`, so Kubernetes API writers can redirect prompts. TEE-held TLS, workload attestation, and the `v0.13.28` GPU boot gate verified. See [review](confidential-ai/review.md). |
