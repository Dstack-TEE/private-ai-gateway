...
| Confidential AI | c8s front door on node-as-CVM (TDX) | Not acceptable | Operator key and host-readable RKE2 state disk both reach cluster-admin inside the TEE (RTMR3 not zero); production runs an unsigned release. TEE-held TLS front door verified 2026-09-23. See [review](confidential-ai/review.md). |
