...
| `middleware.sse_keepalive_ms` | `10000` | Keep-alive interval for streaming responses, measured from the start of the upstream forward. If no upstream response headers have arrived within one interval, the gateway commits `200 text/event-stream` and emits `: PROCESSING` comments at that interval until they do; a later upstream failure is then delivered as the surface's in-band error event (its `code`/`type` is the status the request would otherwise have carried), while the usage report keeps the real status. Once the stream is open the same interval drives idle heartbeats. `0` disables both the early commit and the heartbeat. E2EE responses are never committed early. An early-committed response carries no `x-receipt-id`; fetch the receipt by the response `id`. |
...
does emit a `phase=body_too_large` line (a `413` carrying the request id).
