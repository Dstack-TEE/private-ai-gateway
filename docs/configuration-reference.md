...
| `middleware.sse_keepalive_ms` | `10000` | Idle keep-alive interval for streaming responses; `0` disables the heartbeat (and, with it, `sse_commit_before_upstream`). |
| `middleware.sse_commit_before_upstream` | `false` | Extends the keep-alive to the wait for the upstream's response headers: a streaming request with no upstream answer after one `sse_keepalive_ms` interval is committed as `200 text/event-stream` and heartbeated (`: PROCESSING`) until the upstream responds; a later forward failure is delivered as the surface's in-band error event whose `code`/`type` is the status the request would otherwise have carried, while the usage report keeps the real status. Off by default: a response committed this early carries no `x-receipt-id` header — when the upstream answers and the stream finalizes, the receipt is issued and fetchable by the response `id`, but an early-committed stream whose forward fails never drafts one — which departs from the every-response-carries-a-receipt default. E2EE requests and requests carrying an ACI constraint (`provider.aci_verified` or pinned session ids) are never committed early — their refusal-receipt and 412 semantics only exist as HTTP responses. |
