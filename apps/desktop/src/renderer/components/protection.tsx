...
export function ProtectionStatus({ state, label, detail }: { state: GatewayState; label: string; detail?: string }): React.JSX.Element {
  ...
  return (
    <span className="protection-status inline-flex items-center justify-center gap-1.25 max-w-full flex-wrap [&_>_svg]:flex-none">
      ...
      {detail ? <span className="col-start-2 text-xs leading-4.5 font-normal text-destructive wrap-anywhere" role="alert">{detail}</span> : elapsed !== undefined && <time className="protection-duration w-[8ch] font-medium text-xs leading-4.5 font-mono tabular-nums text-muted-foreground whitespace-nowrap" dateTime={`PT${seconds}S`} aria-label={`Session elapsed ${elapsed}`}>{elapsed}</time>}
    </span>
  );
}
