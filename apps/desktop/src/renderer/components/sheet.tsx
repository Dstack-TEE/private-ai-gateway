...
export function Sheet({ title, label = title, description, className, headingClassName, dismissible = true, initialFocus = "heading", onClose, children }: SheetProps): React.JSX.Element {
  ...
  return <dialog ref={dialog} tabIndex={-1} className={cn("sheet [&.form-sheet]:px-0 fixed left-[var(--window-center-x,_50%)] top-[var(--window-center-y,_50%)] w-[min(410px,_calc(var(--window-dialog-width,_100vw)_-_32px))] max-h-[calc(var(--window-dialog-height,_100vh)_-_32px)] m-0 overflow-auto p-5 text-foreground bg-background border border-border rounded-4xl shadow-xl -translate-x-1/2 -translate-y-1/2 focus:outline-none focus:shadow-xl focus-visible:outline-none focus-visible:shadow-xl [&::backdrop]:bg-[var(--backdrop)] [&_.disclosure]:mt-3 [&_.disclosure]:bg-card [&_.disclosure]:border [&_.disclosure]:border-border [&_.disclosure]:rounded-2xl [&_.disclosure_.sheet-text]:pt-0 [&_.disclosure_.sheet-text]:pr-3.25 [&_.disclosure_.sheet-text]:pb-2.25 [&_.disclosure_.sheet-text]:pl-3.25 [&_.disclosure_.sheet-text]:m-0 max-[440px]:w-[calc(100vw_-_16px)] max-[440px]:max-h-[calc(100vh_-_16px)] max-[440px]:p-4", className)} aria-label={label}>
    <div className={cn("sheet-heading flex items-center gap-3 [&_h2]:text-foreground [&_h2]:text-lg [&_h2]:font-semibold [&_h2:focus]:outline-none [&_h2:focus]:shadow-none", headingClassName)}>
...
export function NativeDialogHost({ className, ...props }: React.ComponentProps<"main">) {
  return <main className={cn("native-dialog-host w-full h-full overflow-hidden bg-background [&_.sheet]:fixed [&_.sheet]:inset-0 [&_.sheet]:w-full [&_.sheet]:max-w-none [&_.sheet]:h-full [&_.sheet]:max-h-none [&_.sheet]:p-5 [&_.sheet]:transform-none [&_.sheet]:translate-none [&_.sheet]:bg-background [&_.sheet]:border-0 [&_.sheet]:rounded-none [&_.sheet]:shadow-none [&_.sheet::backdrop]:bg-transparent [&_.privacy-sheet[open]]:flex [&_.privacy-sheet[open]]:flex-col [&_.privacy-sheet[open]]:overflow-hidden [&_.local-api-sheet[open]]:flex [&_.local-api-sheet[open]]:flex-col [&_.local-api-sheet[open]]:overflow-hidden [&_.usage-evidence-sheet[open]]:flex [&_.usage-evidence-sheet[open]]:flex-col [&_.usage-evidence-sheet[open]]:overflow-hidden [&_.privacy-content]:min-h-0 [&_.privacy-content]:flex-auto [&_.privacy-content]:overflow-auto [&_.sheet_form]:min-h-0 [&_.sheet_form]:flex-auto [&_.sheet_form]:flex [&_.sheet_form]:flex-col [&_.sheet-scroll]:min-h-0 [&_.sheet-scroll]:flex-auto [&_.sheet-scroll]:overflow-auto [&_.sheet-footer]:flex-none [&_.usage-evidence-sheet_.proof-card]:min-h-0 [&_.usage-evidence-sheet_.proof-card]:flex-auto [&_.usage-evidence-sheet_.proof-card]:overflow-auto [&_.usage-evidence-sheet_.proof-card]:pb-3 [&_.privacy-content]:pb-3", className)} {...props} />;
}
