...
function StatusSurface({
  ...
  const activeProfile = state.profiles.find((profile) => profile.id === state.activeProfileId);
  return (
    <Card size="sm" role="region" className={`status-surface relative isolate  [&.status-compact]:transition-colors [&.status-compact]:duration-200 [&.status-compact]:ease-out @max-[600px]/overview:[&.status-compact]:w-full max-[620px]:grid-cols-1 motion-reduce:[&.status-compact]:transition-none status-compact [&_.protected-control]:col-start-2 [&_.protected-control]:row-start-1 [&_.protected-control]:self-start [&_.protected-control]:justify-self-end [&_.protected-control]:min-h-[calc(var(--text-2xl)_*_var(--text-2xl--line-height))] [&_.tracks-right]:inset-0 [&_.tracks-right]:[mask-image:linear-gradient(to_right,_transparent,_#000_12%,_#000_88%,_transparent)] [&_.tracks-right]:text-[color-mix(in_srgb,_var(--muted-foreground)_5%,_var(--card))] [&.status-ready_.tracks-right]:opacity-100 [&_.status-profile]:w-[min(140px,_100%)] [&_.status-profile]:bg-card [&_.is-icon-only]:m-0 [&_.protection-status]:justify-start [&_.status-heading]:transition-colors [&_.status-heading]:duration-200 [&_.status-heading]:ease-out [&_[data-slot=switch]]:transition-colors [&_[data-slot=switch]]:duration-200 [&_[data-slot=switch]]:ease-out motion-reduce:[&_.status-heading]:transition-none motion-reduce:[&_[data-slot=switch]]:transition-none status-${state.status} ${protectedNow ? developmentMode ? "status-ready [&_.tracks-right]:opacity-12 [&_.track-strip]:[animation-play-state:running] ring-warning dark:ring-warning shadow-warning/10" : "status-ready [&_.tracks-right]:opacity-12 [&_.track-strip]:[animation-play-state:running] ring-primary dark:ring-primary shadow-primary/10" : ""} ${developmentMode ? "is-development" : ""}`} aria-label="Protection status">
      <TrackLayer />
