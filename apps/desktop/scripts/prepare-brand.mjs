...
const appIconDefault = await readFile(path.join(brandDir, brand.assets.appIconDefault));
const appIconDark = await readFile(path.join(brandDir, brand.assets.appIconDark));
...
const trayTemplateSvg = composeTrayTemplate(await asset("trayTemplate"));
...
await writeFile(path.join(generatedDir, "app-icon-light.png"), render(pngIconSvg(appIconDefault), 256));
await writeFile(path.join(generatedDir, "app-icon-dark.png"), render(pngIconSvg(appIconDark), 256));
