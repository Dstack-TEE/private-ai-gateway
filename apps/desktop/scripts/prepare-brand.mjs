...
//
//   src/renderer/generated/          brand.ts (copy, theme) and the wordmark SVGs
...
// --- renderer -------------------------------------------------------------
// The mark and wordmarks are copied next to the module and imported as Vite assets,
// so they ship as self-hosted files under the production CSP (`img-src
// 'self'`, no `data:`).
const generatedDir = path.join(appRoot, "src/renderer/generated");
await mkdir(generatedDir, { recursive: true });
await copyFile(path.join(brandDir, brand.assets.appIconMark), path.join(generatedDir, "brand-mark.svg"));
...
const mark = await asset("appIconMark");
...
<rect x="100" y="100" width="824" height="824" rx="185" fill="${brand.theme.iconBackground}"/>
