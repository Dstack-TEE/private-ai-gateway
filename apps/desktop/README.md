...
The Desktop Tauri workflow accepts `production_macos=true`, `release_version`
(a new stable version), and `publish_release=true`. It signs update artifacts,
publishes the desktop release, then calls the shared update-feed workflow to
advance the desktop-only `latest.json`. This explicit call also works before
...
an updater-enabled version. Updates are discovered at startup; installation
