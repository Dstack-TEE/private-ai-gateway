...
internal sealed class NativeTray : IDisposable
{
    ...
    private readonly nint previousProc;
    ...
    internal NativeTray(nint window, Action open, Action settings, Action toggle, Action quit)
    {
        ...
        callback = WndProc;
        previousProc = SetWindowLongPtr(window, GwlpWndProc, Marshal.GetFunctionPointerForDelegate(callback));
    ...
    public void Dispose()
    {
        ...
        SetWindowLongPtr(window, GwlpWndProc, previousProc);
    ...
    [DllImport("user32.dll")] private static extern nint SetWindowLongPtr(nint window, int index, nint value);
    [DllImport("user32.dll")] private static extern nint CallWindowProc(nint previous, nint window, uint message, nuint wParam, nint lParam);
