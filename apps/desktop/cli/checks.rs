...
use private_ai_proxy_aci::digest::{jcs_bytes, sha256_hex, sha256_raw};
use private_ai_proxy_aci::identity;
use private_ai_proxy_aci::keys::verify_receipt_signature;
use private_ai_proxy_aci::receipt::receipt_signing_input;
use private_ai_proxy_aci::types::{AttestationReport, WorkloadKeyset};
...
pub use private_ai_proxy_aci::verifier::{ChannelEvidence, QuoteSource};
...
pub async fn run_report_checks(
    ...
    // appraisal's (`private_ai_proxy_aci::verifier`); this renders the outcomes.
    let appraisal = appraise_report(AppraisalInputs {
        ...
        custody: CustodyEvidence::Unimplemented {
            reason: "custody policy not implemented in this CLI yet \
                     (see apps/desktop/aci/src/verifier/dstack.rs)",
        },
        channel: cx.channel,
        explain: cx.explain,
    })
...
mod tests {
    ...
    fn audit_session_record_agrees_with_the_fixture() {
        ...
        let id = private_ai_proxy_aci::digest::sha256_bare_hex(&bytes);
