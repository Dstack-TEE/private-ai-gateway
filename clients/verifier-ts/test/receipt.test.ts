...
test('§9.2 checks 1–2: a valid receipt document verifies', async () => {
...
test('a tampered document member fails check 1 — the signature covers the content', async () => {
  const bad = structuredClone(fx.ENVELOPE) as ReceiptEnvelope;
  bad.served_at = 1750000001;
  const result = await verifyReceipt(bad, fx.KEYSET, fx.KEYSET_DIGEST);
  assert.equal(result.ok, false);
  assert.equal(check(result.checks, 'signature').ok, false);
...
test('a re-encoded document still verifies — the encoding is free (§7.2)', async () => {
  const reordered: Record<string, unknown> = {};
  for (const key of Object.keys(fx.ENVELOPE).reverse()) {
    reordered[key] = (fx.ENVELOPE as Record<string, unknown>)[key];
  }
  const result = await verifyReceipt(
    reordered as unknown as ReceiptEnvelope,
    fx.KEYSET,
    fx.KEYSET_DIGEST,
  );
  assert.equal(result.ok, true, JSON.stringify(result.checks));
...
test('a non-ed25519 keyset entry is rejected, not silently skipped (Appendix B)', async () => {
  const keyset = structuredClone(fx.KEYSET) as WorkloadKeyset;
  keyset.receipt_signing_keys[0]!.algo = 'frobnitz';
  const result = await verifyReceipt(fx.ENVELOPE, keyset, fx.KEYSET_DIGEST);
  assert.equal(check(result.checks, 'signature').ok, false);
  assert.ok(check(result.checks, 'signature').detail?.includes('unsupported'));
});

test('a document with a foreign api_version is rejected (Appendix B)', async () => {
  const payload = structuredClone(fx.RECEIPT_PAYLOAD);
  payload.api_version = 'aci/2';
  const document = await fx.makeDocument(payload as unknown as Record<string, unknown>);
  const result = await verifyReceipt(document, fx.KEYSET, fx.KEYSET_DIGEST);
  assert.equal(result.ok, false);
...
test('a document missing its signature fails check 1', async () => {
  const bad = structuredClone(fx.ENVELOPE) as ReceiptEnvelope;
  bad.signature = '';
  const result = await verifyReceipt(bad, fx.KEYSET, fx.KEYSET_DIGEST);
  assert.equal(result.ok, false);
...
test('unknown extension event types are ignored (§7.4)', async () => {
  const payload = structuredClone(fx.RECEIPT_PAYLOAD);
...
test('§9.2 checks 3–4: body-hash helpers match the receipt events', async () => {
