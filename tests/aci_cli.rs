...
async fn audit_verifies_artifacts_captured_from_a_live_service() {
    ...
    let receipt_bytes = http
        ...
        .await
        .unwrap();
    // The endpoint serves the §7.2 receipt document: the event log is
    // readable directly.
    let payload: Value = serde_json::from_slice(&receipt_bytes).unwrap();
    ...
    assert_eq!(check_status(&transcript, "id-1"), "fail");
    assert_eq!(check_status(&transcript, "id-4"), "skip");
    assert_eq!(check_status(&transcript, "id-5"), "skip");
    assert_eq!(check_status(&transcript, "id-6"), "skip");
...
fn audit_verifies_the_checked_in_report_fixture() {
    ...
    assert_eq!(check_status(&transcript, "id-2"), "pass");
    assert_eq!(check_status(&transcript, "id-3"), "skip");
    // No parseable quote / no app_compose from the stub: id-4 skips honestly.
    assert_eq!(check_status(&transcript, "id-4"), "skip");
...
/// A report in the pre-simplification shape (identity key, endorsement,
/// epoch-style keyset) must fail closed. It parses — unknown members are
/// ignored (Appendix B) — and then fails the id-2 digest recompute, because
/// its keyset does not hash to the digest it claims.
#[test]
